...
BOOST_FIXTURE_TEST_CASE(get_set_gage_rate, FixtureBeforeStep){
    ...
    do
    {
        ...
        {
            // Gage
            error = swmm_getGagePrecip(rg_ind, &rain, &snow, &total);
            BOOST_REQUIRE(error == ERR_NONE);
            BOOST_CHECK_SMALL(rain - start_rainfall_rate, 0.0001);
            BOOST_CHECK_SMALL(snow - 0.0, 0.0001);
            BOOST_CHECK_SMALL(total - start_rainfall_rate, 0.0001);
        ...
        if (step_ind == 720) // (Jan 1, 1998 12:00pm)
        {
            ...
            BOOST_CHECK_SMALL(snow - 0.0, 0.0001);
            BOOST_CHECK_SMALL(total - new_rainfall_rate, 0.0001);
